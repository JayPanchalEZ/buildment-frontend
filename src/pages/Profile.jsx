...
function Profile({ setDarkMode, darkMode }) {
  ...
  const userProfileRef = useRef({ name: "", email: "", picture: "" });
  ...
  useEffect(() => {
    ...
    if (!storedUser) {
      ...
      userProfileRef.current = storedUser;
      setProfilePic(storedUser.picture || "default-avatar.png");
    }
  }, [navigate]);
  ...
  const handleImageUpload = (event) => {
    ...
    const imageUrl = URL.createObjectURL(file); // Create object URL for faster loading
    setProfilePic(imageUrl);

    // Store in localStorage
    const updatedUser = { ...userProfileRef.current, picture: imageUrl };
        ...
        >
          ...
          <div className="text-center">
            <div className="relative inline-block">
              <img
                src={profilePic}
                alt={userProfileRef.current.name || "Profile"}
            ...
            <h2 className="mt-4 text-2xl font-semibold text-gray-800 dark:text-white">{userProfileRef.current.name}</h2>
            <p className="text-gray-500 dark:text-gray-400">{userProfileRef.current.email}</p>
          </div>
            ...
            <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 dark:bg-gray-700 dark:border-gray-600">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <p className="mt-1 text-gray-900 font-medium dark:text-gray-300">{userProfileRef.current.name}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-600 dark:text-gray-400">Email</label>
                  <p className="mt-1 text-gray-900 font-medium dark:text-gray-300">{userProfileRef.current.email}</p>
                </div>
              </div>
            </div>
          </div>
